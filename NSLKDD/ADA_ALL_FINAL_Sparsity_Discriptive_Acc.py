...
def get_explanation(instance):
    ...
    exp = explainer.explain_instance(instance, lambda x: multi_target_abc.predict(x.reshape(-1, len(instance))), num_features=len(X_train.columns))
...
explainer = shap.KernelExplainer(multi_target_abc.predict, shap.sample(X_train, 100))
...
def get_explanation(instance):
    exp = explainer.explain_instance(instance, lambda x: multi_target_abc.predict(x), num_features=len(X_train.columns))
    return dict(exp.as_list())
...
# Assuming necessary data preparation steps such as initializing X_train, Y_train, class_names, etc.
...
# Assuming misclassified_instance_df is a DataFrame containing the instances you want to explain
explanations = [get_explanation(instance) for instance in X_test[:2].values]
...
# Get the features sorted by their average importance in descending order
sorted_features = sorted(global_importances.items(), key=lambda x: -x[1])

# Print the sorted global importances
...
array1 = get_top_features(multi_target_abc, shap.sample(X_train, 100), small_X_test, X_train.columns)
array2 = get_top_features(multi_target_abc, shap.sample(X_train, 100), small_X_test, X_train.columns)
array3 = get_top_features(multi_target_abc, shap.sample(X_train, 100), small_X_test, X_train.columns)
...
array1 = get_top_lime_features(multi_target_abc, X_train.values, small_X_test, X_train.columns)
print(array1)
array2 = get_top_lime_features(multi_target_abc, X_train.values, small_X_test, X_train.columns)
print(array2)
array3 = get_top_lime_features(multi_target_abc, X_train.values, small_X_test, X_train.columns)
