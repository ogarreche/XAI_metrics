...
def get_explanation(instance):
    ...
    exp = explainer.explain_instance(instance, lambda x: multi_target_clf.predict(x.reshape(-1, len(instance))), num_features=len(X_train.columns))
...
# Use KernelExplainer for model agnostic
explainer = shap.KernelExplainer(multi_target_clf.predict, shap.sample(X_train, 100))
...
def get_explanation(instance):
    exp = explainer.explain_instance(instance, lambda x: multi_target_clf.predict(x), num_features=len(X_train.columns))
    return dict(exp.as_list())
...
array1 = get_top_features(multi_target_clf, shap.sample(X_train, 50), small_X_test, X_train.columns)
array2 = get_top_features(multi_target_clf, shap.sample(X_train, 50), small_X_test, X_train.columns)
array3 = get_top_features(multi_target_clf, shap.sample(X_train, 50), small_X_test, X_train.columns)
...
array1 = get_top_lime_features(multi_target_clf, X_train.values, small_X_test, X_train.columns)
print(array1)
array2 = get_top_lime_features(multi_target_clf, X_train.values, small_X_test, X_train.columns)
print(array2)
array3 = get_top_lime_features(multi_target_clf, X_train.values, small_X_test, X_train.columns)
