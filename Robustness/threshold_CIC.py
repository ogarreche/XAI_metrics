...
categorical_features = [i for i,f in enumerate(features) if f not in [' Init_Win_bytes_backward', ' Fwd Packet Length Std', ' Flow IAT Max']]
###
...
def experiment_main(X, y):
	...
	if LIME:
		...
		limeorshap = 'shap'
	df.to_csv('data/threshold_results_CIC_{}.csv'.format(limeorshap))
