...
output_file_name = "MLP_SHAP_SML.txt"
...
start = time.time()
MLP = MLPClassifier(random_state=5, max_iter=max_iter,early_stopping=early_stopping ).fit(X_train, y_train)
end = time.time()
...
with open(output_file_name, "a") as f: print('y_axis_MLP = ', Spar ,'', file = f)

with open(output_file_name, "a") as f:print('x_axis_MLP = ', X_axis ,'', file = f)
